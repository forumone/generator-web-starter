...
PROJECT_ROOT="/var/www/vhosts/project_name.dev"		# WordPress remote environment project root.
# ======================================== END REMOTE CONFIG ======================================== #


# ======================================== COMMON CONFIG ======================================== #
WEB_ROOT_FOLDER="public"	# Where the WordPress files lives, commonly this would be 'public' or 'web'.
SCRIPT_BASE_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
VM_WEB_ROOT=$SCRIPT_BASE_DIR"/../"$WEB_ROOT_FOLDER"/"
...
VAGRANT_VM_WEB_ROOT="/vagrant/"$WEB_ROOT_FOLDER"/"
# ======================================== END COMMON CONFIG ======================================== #
...
function f1_run {
	REMOTE_WEB_ROOT=$PROJECT_ROOT"/"$WEB_ROOT_FOLDER"/"

	# REMOTE ENV WORKFLOW:
	# 1- Exports the DB on remote environment.
	echo ">>> Connecting to $USER_AT_HOST:$PROJECT_ROOT environment and exporting the database..." &&
	SSH_OUTPUT_RESULT=$( ssh $USER_AT_HOST "cd $REMOTE_WEB_ROOT &&
	wp db export $PROJECT_ROOT/db_export_$( date +%Y-%m-%d_%H-%M-%S ).sql &&
	exit"
	) &&
	# 2- Extract the .sql file name from $SSH_OUTPUT_RESULT.
	SQL_FILE_PATH=$( echo $SSH_OUTPUT_RESULT | awk -F[\'\'] '{print $2}' ) &&
	SQL_FILE_NAME=$( basename "$SQL_FILE_PATH" ) &&
	# 3- SCP download to the local Vagrant VM the .sql file.
	echo ">>> Downloading $SQL_FILE_NAME from $USER_AT_HOST:$PROJECT_ROOT environment..." &&
	scp $USER_AT_HOST":"$PROJECT_ROOT"/"$SQL_FILE_NAME $VM_WEB_ROOT &&
	# 4- Removes the .sql file from remote environment.
	echo ">>> Removing $SQL_FILE_NAME from $USER_AT_HOST:$PROJECT_ROOT environment..." &&
	ssh $USER_AT_HOST "cd $PROJECT_ROOT &&
	rm $SQL_FILE_NAME &&
	exit" &&
